import { CargoTypesUnion } from '@/types/cargo-types'
import { ExpenseStatusTypeUnion } from '@/types/expense-types'
...
function getAllVariantCombinations(variantGroups: VariantItem[][]): { combination: string; variant_ids: string[] }[] {
...
export function getVariantPreview(productName: string, variantGroups: VariantItem[]): Array<{ name: string; variants: string[] }> {
...
export const cargoTypesCheckIn = (checks: Array<CargoTypesUnion>, value: CargoTypesUnion): boolean => {
  return checks.includes(value)
}

export const expenseStatusCheckIn = (checks: Array<ExpenseStatusTypeUnion>, value: ExpenseStatusTypeUnion): boolean => {
  return checks.includes(value)
}
