...
} from '@/services/api/sub-product-api'
		...
		if (data) {
			data.results.then((product: ProductDataType) => {
				productUpdateFormik.setValues({
					...
					buy_currency: {
						label: product.sub_products[0].buy_currency.name,
						value: product.sub_products[0].buy_currency.id,
					},
					sell_currency: {
						label: product.sub_products[0].sell_currency.name,
				...
				product.sub_products.map((item, index) => {
					alreadyCreatedVariantsFormik.setFieldValue(`data.${index}`, {
						...
						sell_price: item.sell_price,
						buy_currency: {
							label: item.buy_currency.name,
							value: item.buy_currency.id,
						},
						sell_currency: {
							label: item.sell_currency.name,
								...
								<div className={'flex-col col-span-6'}>
										...
										<Card.Header>
											<div className={'flex items-center justify-between flex-1'}>
												...
												<LugatButton
													...
													onClick={productUpdateFormik.submitForm}
												>
											...
											<SeperatedColumn>
												<SeperatedRow>
													<LugatInput
														...
														onChange={(e) =>
															productUpdateFormik.setFieldValue('name', e.target.value)
														}
														error={
															productUpdateFormik.touched.name && productUpdateFormik.errors.name
														}
													/>
												</SeperatedRow>
												...
												<SeperatedRow>
													<LugatInput
														...
														onChange={(e) =>
															productUpdateFormik.setFieldValue('sku', e.target.value)
														}
														error={
															productUpdateFormik.touched.sku && productUpdateFormik.errors.sku
														}
													/>
													<LugatInput
														label={'Barcode'}
														...
														onChange={(e) =>
															productUpdateFormik.setFieldValue('barcode', e.target.value)
														}
														error={
															productUpdateFormik.touched.barcode &&
															productUpdateFormik.errors.barcode
														}
													/>
												</SeperatedRow>
												<SeperatedRow>
													<LugatCurrencyInput
														label={'Buy Price (based on first variant)'}
														disabled
														required
														error={
															getIn(productUpdateFormik.touched, 'buy_price') &&
															getIn(productUpdateFormik.errors, 'buy_price')
														}
													...
													<LugatAsyncSelect
														...
														label={'Buy Currency (based on first variant)'}
														additional={{
															page: 1,
														}}
														placeholder={'Select'}
												...
												<SeperatedRow>
													<LugatCurrencyInput
														label={'Sell Price (based on first variant)'}
														disabled
														required
														error={
															getIn(productUpdateFormik.touched, 'sell_price') &&
															getIn(productUpdateFormik.errors, 'sell_price')
														}
												...
												<LugatToggle
													...
													onChange={(val) =>
														productUpdateFormik.setFieldValue('have_variants', val)
													}
												/>
											</SeperatedColumn>
										</Card.Body>
									...
									>
										<Card>
												...
												<SeperatedColumn>
													...
													{variantFormik.values.data.length > 0 && (
														<div className={'flex-1 flex w-full'}>
															<LugatButton onClick={handleGenerateVariant}>
																Generate Variants
															</LugatButton>
														</div>
													)}
											...
											<Card.Footer>
												<LugatButton onClick={newlyCreatedVariantsFormik.submitForm}>
													{!isLoading ? 'Save' : <LoaderIcon />}
												</LugatButton>
											</Card.Footer>
										</Card>
